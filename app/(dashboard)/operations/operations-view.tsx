...
type OpsData = {
  ...
  repeaters: ExemptRepeater[];
  analystScenarios: AnalystScenario[];
  campuses: Campus[];
  routeRows: RouteMetricRow[];
  hotspotRows: HotspotMetricRow[];
  repeaterRows: RepeaterMetricRow[];
};

export default function OperationsView({ data }: { data: Promise<OpsData> }) {
  const resolved = use(data);

  const [routeComparisons, setRouteComparisons] = useState<RouteComparison[]>(resolved.routes || []);
  const [analystScenarios] = useState<AnalystScenario[]>(resolved.analystScenarios || []);
  const [curatedRepeaters] = useState<ExemptRepeater[]>(resolved.repeaters || []);
  const [campuses] = useState<Campus[]>(resolved.campuses || []);
  const [enabledRoutes, setEnabledRoutes] = useState<Record<string, boolean>>(() => {
    const next: Record<string, boolean> = {};
    (resolved.routes || []).forEach((route) => {
  ...
  const [routeMetrics] = useState<RouteMetricRow[]>(resolved.routeRows || []);
  const [hotspotMetrics] = useState<HotspotMetricRow[]>(resolved.hotspotRows || []);
  const [repeaterMetrics] = useState<RepeaterMetricRow[]>(resolved.repeaterRows || []);
  ...
  const comparisonData = useMemo(() => {
    return filteredRouteMetrics
      .slice()
      .sort((a, b) => (a.violations ?? 0) < (b.violations ?? 0) ? 1 : -1)
          ...
          <div className="surface-card rounded-xl border border-foreground/10 bg-card/80 p-4 shadow-soft-lg space-y-3 sm:p-5">
            ...
            <ul className="space-y-3 text-sm text-foreground/80">
              {filteredRepeaters.slice(0, 6).map((repeater) => {
                const staticContext = (repeater as any).staticContext;
                return (
                  <li
          ...
          <div className="surface-card rounded-xl border border-foreground/10 bg-card/80 p-4 shadow-soft-lg space-y-3 sm:p-5">
            ...
            <ul className="space-y-3 text-sm text-foreground/80">
              {analystScenarios.map((scenario) => (
                <li
                  key={scenario.title}
                  className="rounded-xl border border-foreground/10 bg-background/80 p-3 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-lg"
                >
            ...
            <tbody className="divide-y divide-foreground/10">
              {routeTable.map((route) => (
                <tr key={route.routeId} className="align-top transition-colors hover:bg-primary/5">
                  ...
                  <td className="py-2 pr-3">{integer.format((route as any).dynamicAverageMonthlyViolations)}</td>
                  <td className="py-2 pr-3">{formatPercentValue((route as any).dynamicExemptSharePct)}</td>
